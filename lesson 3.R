...
heights = greendb %>%group_by(adm_region) %>% 
  summarise(
    ...
    height_mean = mean(height_m, na.rm=T)
  ) %>% group_by(adm_region) %>%
  arrange(adm_region, desc(height_mean)) %>%
  mutate(order = order(height_mean, decreasing = T)) %>%
  filter( order == 1) %>% select(-order, -height_mean) %>%
  rename(NAME = adm_region)
